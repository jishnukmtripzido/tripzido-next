...
export default function PopularRentals() {
  ...
  const maxIndex = BIKES.length - VISIBLE;
  ...
  function setCardWidths() {
    ...
    const gap = 20;
    const cardW = (viewportRef.current.offsetWidth - gap * (VISIBLE - 1)) / VISIBLE;
  ...
  function updateTransform(idx: number) {
    if (isMobile() || !trackRef.current) return;
    const gap = 20;
    const first = trackRef.current.children[0] as HTMLElement;
    const cardW = parseFloat(first?.style.width ?? "0");
  ...
  useEffect(() => {
    ...
    const onResize = () => { setIndex(0); setCardWidths(); updateTransform(0); };
    ...
    <section className="pt-12 px-4 lg:px-8 mx-auto xl:mx-[121.5px] xl:px-0">
      ...
      <div className="relative group">
        ...
        <button
          ...
          className="hidden md:flex absolute -left-5 top-1/2 -translate-y-1/2 w-10 h-10 bg-white shadow-lg border border-gray-100 rounded-full items-center justify-center text-gray-400 hover:text-black z-30 disabled:opacity-30"
        >
        ...
        <button
          ...
          className="hidden md:flex absolute -right-5 top-1/2 -translate-y-1/2 w-10 h-10 bg-white shadow-lg border border-gray-100 rounded-full items-center justify-center text-gray-400 hover:text-black z-30 disabled:opacity-30"
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div className="border border-gray-300 rounded-xl overflow-hidden flex-shrink-0 min-w-[240px] md:min-w-0">
      ...
      <div className="relative h-48 bg-white overflow-hidden flex items-center justify-center p-3">
        <img
          ...
          onError={(e) => {
            (e.target as HTMLImageElement).src = `https://placehold.co/400x200/f3f4f6/9ca3af?text=${encodeURIComponent(bike.name)}`;
          }}
        />
        {bike.badge && (
          <span
      ...
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-1.5 my-3">
          {bike.tags.map((tag) => (
            <span key={tag} className="text-[11px] bg-gray-100 text-gray-600 rounded-full px-2 py-0.5 font-medium">
              {tag}
            </span>
          ))}
        </div>
