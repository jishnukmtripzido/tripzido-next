...
//   // Mount Turnstile widget when modal opens (only on phone step)
//   useEffect(() => {
...
//   // Cleanup Turnstile on close or when moving to OTP step
//   useEffect(() => {
//     if (!isOpen || otpSent) {
...
//     try {
//       const response = await fetch(process.env.NEXT_PUBLIC_API_URL + "/api/users/send-otp/", {
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({ phone_number: phone, turnstile_token: token }),
//       });
...
//     setOtpSent(false);
//     setOtp(["", "", "", ""]);
//     // Turnstile will re-mount via useEffect when otpSent becomes false
//   };
...
//                   {/* Turnstile only shown on phone step */}
...
import { verifyOtpAndLogin } from "@/app/actions/auth";
...
export default function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  // Mount Turnstile when modal opens on the phone step
  useEffect(() => {
    if (!isOpen || otpSent) return;
  ...
  // Cleanup Turnstile when modal closes or moves to OTP step
  useEffect(() => {
    if (!isOpen || otpSent) {
      if (turnstileWidgetId.current && window.turnstile) {
        window.turnstile.remove(turnstileWidgetId.current);
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        // Shake the boxes / clear for retry
        setOtp(["", "", "", ""]);
      ...
      handleClose();
      router.refresh(); // Re-renders Server Components in place — Header picks up the new cookie
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResend = () => {
    ...
    setOtp(["", "", "", ""]);
    setOtpError(null);
    // Turnstile re-mounts via useEffect when otpSent becomes false
  };
