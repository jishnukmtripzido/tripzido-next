          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-lg overflow-visible">
            <div className="p-4 pb-0">
              ...
              >
                ...
                >
                  <svg className="text-gray-500 shrink-0" width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" strokeLinecap="round" strokeLinejoin="round" />
                    <circle cx="12" cy="10" r="3" />
                  </svg>
                  <span className="flex flex-col flex-1 min-w-0 py-0.5">
                ...
                <div
                  className={`absolute left-0 right-0 bg-white border border-gray-200 shadow-xl overflow-hidden transition-all duration-200 ease-in-out z-50 ${
                    ...
                    dropUp
                      ? "bottom-full border-b-0 rounded-t-lg origin-bottom"
                      : "top-full border-t-0 rounded-b-lg origin-top"
                  }`}
                >
                    ...
                    {locations.map((loc) => {
                      ...
                      return (
                        <div
                          key={loc.id}
                          onClick={() => handleSelectLocation(loc)}
                          className={`flex items-center justify-between px-4 py-3 cursor-pointer text-[13px] transition-colors ${
                            isSelected ? "bg-gray-50 font-medium" : "hover:bg-gray-50/60"
                          }`}
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            {/* Simple clean status indicator like image */}
                            <span className="w-3 h-3 rounded-[3px] bg-green-500 shrink-0" />
                            <div className="flex flex-col min-w-0">
                              <span className={`truncate text-black ${isSelected ? "font-semibold" : ""}`}>
                                {loc.location_name}
                              </span>
                              <span className="text-[11px] text-gray-400 mt-0.5">
                                {locPrice !== null ? `₹${locPrice.toLocaleString("en-IN")}/day` : "No price"}
                              </span>
                            </div>
                          </div>
                          {/* Premium radio select icon from your graphic preference */}
              ...
              <div className="px-4 pb-4 flex flex-col flex-1 justify-between">
                ...
                >
                  <div
                    ref={triggerRef}
                    onClick={handleTriggerClick}
                    className={`w-full flex items-center gap-2 border border-gray-300 pl-3 py-1 cursor-pointer bg-white transition-all overflow-hidden ${
                      dropdownOpen
                        ? `border-gray-400 rounded-[10px] ${dropUp ? "" : "rounded-b-none"}`
                        : "border-gray-300 rounded-[10px] hover:border-gray-400"
                    }`}
                  >
                    <svg className="text-gray-500 shrink-0" width="16" height="16" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  <div
                    className={`absolute left-0 right-0 bg-white border border-gray-200 shadow-xl overflow-hidden transition-all duration-200 ease-in-out z-50 ${
                      ...
                      dropUp
                        ? "bottom-full border-b-0 rounded-t-[10px] origin-bottom"
                        : "top-full border-t-0 rounded-b-[10px] origin-top"
                    }`}
                  >
                    <div className="max-h-60 overflow-y-auto divide-y divide-gray-100">
                      {locations.map((loc) => {
                        ...
                        return (
                          <div
                            ...
                            onClick={() => handleSelectLocation(loc)}
                            className={`flex items-center justify-between px-4 py-3 cursor-pointer text-[13px] transition-colors ${
                              isSelected ? "bg-gray-50 font-medium" : "hover:bg-gray-50/50"
                            }`}
                          >
                            <div className="flex items-center gap-3 min-w-0">
                              <span className="w-3 h-3 rounded-[3px] bg-green-500 shrink-0" />
