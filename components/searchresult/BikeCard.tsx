...
//   const [selectedLocation, setSelectedLocation] = useState(location_name);
...
//   const imageUrl = images.find((img) => img.is_primary)?.image ?? images[0]?.image ?? vehicle_type.primary_image;
//   const transmission = vehicle_type.transmission_type === "AUTOMATIC" ? "Automatic" : "Manual";
//   const seats = `${vehicle_type.seats} Seater`;
//   const fuelType = vehicle_type.fuel_type.charAt(0) + vehicle_type.fuel_type.slice(1).toLowerCase();
//   const engine = `${vehicle_type.cc} cc`;
//   const mileage = `${vehicle_type.mileage_kmpl} kmpl`;
//   const price = pricing_packages.length > 0
//     ? Math.min(...pricing_packages.map((p) => parseFloat(p.price_per_day)))
//     : null;
...
//   return (
//     <div className="group relative w-full max-w-sm h-[410px] bg-transparent">
//       <div style={{ perspective: "1000px" }} className="relative w-full h-full">
//         <div className={`relative w-full h-full transition-all duration-700 [transform-style:preserve-3d] ${isFlipped ? "[transform:rotateY(180deg)]" : ""}`}>

//           {/* FRONT FACE */}
//           <div className={`absolute inset-0 w-full h-full bg-white border border-gray-300 rounded-xl hover:shadow-sm transition-shadow [backface-visibility:hidden] flex flex-col pt-4 ${isFlipped ? "pointer-events-none" : "pointer-events-auto"}`}>
//             <h3 className="font-medium text-black text-[16px] leading-none text-center px-4 tracking-tight">{name}</h3>
...
//                 onError={(e) => { (e.target as HTMLImageElement).src = `https://placehold.co/300x160/f3f4f6/9ca3af?text=${encodeURIComponent(name)}`; }}
//               />
//             </div>

//             <div className="relative flex items-center justify-center px-4 mt-1 mb-3 z-10">
//               <div className="flex-1 border-t-2 border-gray-200"></div>
//               <button onClick={() => setIsFlipped(true)} className="px-4 py-1 bg-white border border-gray-200 rounded-full text-[11px] font-thin text-black mx-3 hover:border-[#ffc107] transition-colors cursor-pointer shadow-sm">
//                 View Specs
//               </button>
//               <div className="flex-1 border-t-2 border-gray-200"></div>
//             </div>
...
//                 </div>
//               </div>
//               <div className="flex justify-between items-center text-black text-[12px] font-normal px-1 mt-2">
//                 <div className="flex items-center gap-1.5">
//                   <svg className="w-3.5 h-3.5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
//                     <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
//                   </svg>
...
//               {/* Location dropdown */}
//               <div className="relative mt-3" ref={dropdownRef} style={{ zIndex: dropdownOpen ? 50 : "auto" }}>
//                 <div ref={triggerRef} onClick={handleTriggerClick}
//                   className={`w-full flex items-center gap-2 border px-3 py-2 cursor-pointer bg-white transition-colors ${dropdownOpen ? `border-[#ffc107] rounded-[10px] ${dropUp ? "" : "rounded-b-none"}` : "border-gray-200 rounded-[10px] hover:border-[#ffc107]"}`}>
//                   <svg className="text-[#ffc107] shrink-0" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
...
//                     <span className="text-[13px] font-thin text-black leading-snug truncate mt-1">{selectedLocation}</span>
//                   </span>
//                   <svg className={`shrink-0 transition-transform duration-200 ${dropdownOpen ? "rotate-180 text-[#ffc107]" : "text-gray-300"}`} width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
//                     <path d="M6 9l6 6 6-6" strokeLinecap="round" strokeLinejoin="round" />
//                   </svg>
//                 </div>

//                 {dropdownOpen && (
//                   <div className={`absolute left-0 right-0 bg-white border border-[#ffc107] overflow-hidden shadow-lg ${dropUp ? "bottom-full border-b-0 rounded-t-[10px]" : "top-full border-t-0 rounded-b-[10px]"}`}>
//                     {/* Single location from API for now */}
//                     <div
//                       onClick={() => { setSelectedLocation(location_name); setDropdownOpen(false); onDropdownOpenChange?.(false); }}
//                       className="flex items-center gap-2.5 px-3 py-2 cursor-pointer text-[12.5px] bg-amber-50 text-black"
//                     >
//                       <svg className="text-[#ffc107]" width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
//                         <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" strokeLinecap="round" strokeLinejoin="round" /><circle cx="12" cy="10" r="3" />
//                       </svg>
//                       {location_name}
...
//                       <span className="text-black font-medium text-[19px] leading-none">₹{price.toLocaleString("en-IN")}</span>
...
//           {/* BACK FACE */}
//           <div className={`absolute inset-0 w-full h-full bg-white border border-gray-200 rounded-xl shadow-sm [backface-visibility:hidden] [transform:rotateY(180deg)] flex flex-col overflow-hidden pt-4 ${isFlipped ? "pointer-events-auto" : "pointer-events-none"}`}>
//             <h3 className="font-medium text-black text-[16px] leading-none text-center px-4 tracking-tight">{name}</h3>

//             <div className="relative h-36 flex items-center justify-center mt-2 px-8">
//               <img src={imageUrl} alt={name} className="h-full w-full object-contain opacity-90 drop-shadow-sm"
//                 onError={(e) => { (e.target as HTMLImageElement).src = `https://placehold.co/300x160/f3f4f6/9ca3af?text=${encodeURIComponent(name)}`; }} />
//             </div>

//             <div className="relative flex items-center justify-center px-4 mt-1 mb-3 z-10">
//               <div className="flex-1 border-t border-gray-200"></div>
//               <button onClick={() => setIsFlipped(false)} className="px-4 py-1 bg-white border border-gray-200 rounded-full text-[11px] font-semibold text-gray-600 mx-3 hover:border-[#ffc107] hover:text-black transition-colors cursor-pointer shadow-sm">
//                 ← Back
//               </button>
//               <div className="flex-1 border-t border-gray-200"></div>
//             </div>
...
//                 <span className="text-[16px] font-semibold text-black">{vehicle_type.make_year}</span>
//               </div>
...
//                 <span className="text-[16px] font-semibold text-black">{km_limit_per_day}</span>
//               </div>
...
function getLocationPrice(loc: VehicleLocation): number | null {
  // Prefer pricing_packages minimum, fall back to daily_price
  if (loc.pricing_packages.length > 0) {
    return Math.min(...loc.pricing_packages.map((p) => parseFloat(p.price_per_day)));
  }
  if (loc.daily_price !== null) {
    return parseFloat(loc.daily_price);
  }
  return null;
}
...
export default function BikeCard({
  ...
  // Derived display values (vehicle-level, not location-level)
  const imageUrl = primary_image;
    // selectedLocation.images.find((img) => img.is_primary)?.image ??
    // selectedLocation.images[0]?.image ??
    
  const transmission = transmission_type === "AUTOMATIC" ? "Automatic" : "Manual";
  ...
  // Location-specific derived values (update when location changes)
        ...
        >
          ...
          >
            ...
            <div className="relative flex items-center justify-center px-4 mt-1 mb-3 z-10">
              ...
              <button
                ...
                className="px-4 py-1 bg-white border border-gray-200 rounded-full text-[11px] font-thin text-black mx-3 hover:border-[#ffc107] transition-colors cursor-pointer shadow-sm"
              >
                View Specs
              </button>
              <div className="flex-1 border-t-2 border-gray-200"></div>
            ...
            <div className="px-4 pb-4 flex flex-col flex-1 justify-between">
              ...
              {/* Price & Book — reflects selected location */}
              <div className="flex justify-between items-end border-t-2 border-gray-200 pt-3 mt-3">
                <div>
                  {price !== null ? (
                    <div className="flex items-baseline gap-0.5">
          ...
          >
            ...
            <div className="flex-1 grid grid-cols-2 grid-rows-2">
              ...
              <div className="border-t border-gray-100 flex flex-col items-center justify-center p-3 bg-white">
                <span className="text-[12px] font-thin text-black mb-0.5">Km/day limit</span>
                <span className="text-[16px] font-semibold text-black">{kmLimit}</span>
              </div>
            </div>
          </div>
        </div>
