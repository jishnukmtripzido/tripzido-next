...
function CheckItem({
  ...
  return (
    <label className="flex items-center gap-2 text-sm text-black cursor-pointer py-0.5">
      ...
      <span className="text-[0.79rem] text-black">{label}</span>
      <span className="ml-auto text-xs text-black">{count}</span>
    </label>
  );
}
...
export default function FilterSidebar({
  ...
  const visiblePriceRanges = PRICE_RANGE_META.filter((r) => priceRangeCounts[r.key] != null);
  const visibleCcRanges = CC_RANGE_META.filter((r) => ccRangeCounts[r.key] != null);
