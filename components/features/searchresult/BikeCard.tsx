...
//         <div className="bg-white rounded-md shadow-[0px_1px_2px_0px_rgba(60,64,67,0.3),0px_2px_6px_2px_rgba(60,64,67,0.15)] overflow-visible pb-2">
...
export default function BikeCard({
  ...
  const PriceDisplay = () => (
    <div>
      {rentalDays && totalPrice !== null ? (
        <div className="flex flex-col items-start gap-0.5">
          <span className="text-[11px] font-normal text-font-dim">
            Price for {rentalDays} day{rentalDays > 1 ? "s" : ""} :
          </span>
          <span className="text-xl mt-1.5 md:mt-1 md:text-lg font-bold text-black leading-none">
  ...
  const SpecsList = () => (
    <ul className="space-y-2.5 text-font-main-sub">
      {[
        { icon: "user", label: seatsLabel },
        { icon: "gear", label: transmission },
        { icon: "map", label: kmLimit ? `${kmLimit}` : "Unlimited Km" },
        ...
        <li
          key={label}
          className="flex items-center gap-2 text-[13px] font-normal text-gray-700 leading-none"
        >
          <SpecIcon type={icon} />
          <span className="truncate">{label}</span>
        </li>
  ...
  // Mobile-only badge row: tags + pay-at-pickup + electric, unified into a
  // single flex row so spacing between them is consistent instead of each
  // badge carrying its own ad-hoc margin.
  const Badges = () => {
    const hasBadges =
      tags.length > 0 ||
      selectedLocation.pay_at_pickup_enabled ||
    ...
    return (
      <div className="flex flex-wrap items-center gap-1.5 mb-3">
        {tags.map((t, i) => (
          <span
            key={i}
            className={`text-[11px] font-semibold px-2 py-0.5 rounded ${tagClass(t.variant)}`}
          >
            {t.label}
          </span>
        ))}
        {selectedLocation.pay_at_pickup_enabled && (
          <span className="inline-flex items-center bg-green-700 text-white text-[11px] font-semibold px-2 py-0.5 rounded">
            Pay at pickup
          </span>
        )}
        {fuel_type === "ELECTRIC" && (
          <span className="inline-flex items-center bg-brand-yellow text-[#6b3d00] text-[11px] font-semibold px-2 py-0.5 rounded">
            Electric
          </span>
        )}
      </div>
    ...
    <div className="group relative w-full max-w-sm sm:max-w-none bg-transparent">
      ...
      <div className="sm:hidden">
        <div className="bg-white rounded-xl border border-gray-200/80 shadow-sm overflow-visible pb-2">
          <div className="p-4 pb-0">
            <Badges />
            <h3 className="text-[17px] font-bold text-gray-900 leading-snug tracking-tight mb-2.5">
              {name}
            </h3>
            <div className="grid grid-cols-[1fr_140px] gap-3 items-center mb-5">
              <div className="min-w-0">
                <SpecsList />
              </div>
              {/* Mobile bike image */}
              <div className="flex justify-end">
                <BikeImage
                  ...
                  height={115}
                  sizes="140px"
                  className="w-[140px] h-[115px] object-contain -mt-8"
                />
              </div>
            </div>
            <LocationDropdown
          ...
          <div className="flex items-center justify-between px-4 py-3.5 mt-3 border-t border-gray-100">
            <PriceDisplay />
            <BookButton />
          </div>
        </div>
