...
export default function EditSearchBar({
  ...
  // Reverts every editable field (location, dates, times) back to the
  // last confirmed-good search — the values this component originally
  // mounted with via initialLocationName/initialPickup/initialDropoff.
  // Used when a search resolves successfully but comes back with no
  // matching listing, so the bar doesn't strand the vendor on an
  // invalid combination.
  function resetToInitial() {
    setSelectedLocationId(
      String(
        cityLocations.find((l) => l.location_name === initialLocationName)
          ?.id ?? "",
      ),
    );
    setLocationError(null);
    handleDateSelect({
      start: parseDate(initialPickup),
      end: parseDate(initialDropoff),
    });
    setPickupTime({
      hour: parseHour(initialPickup),
      minute: parseMinute(initialPickup),
    });
    setDropoffTime({
      hour: parseHour(initialDropoff),
      minute: parseMinute(initialDropoff),
    });
    setDateErrors({});
  }
