...
export default function VehicleFeatures({
  ...
  const features = [
    { label: "", value: `${seats} Seater`, icon: UserGroupIcon },
  ...
  return (
    <div className="mt-2 md:mt-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 md:gap-y-4 gap-x-4 md:gap-x-8">
        {features.map(({ label, value, icon: Icon, hideOnMobile }) => (
          <div
            key={label}
            className={`${hideOnMobile ? "hidden md:flex" : "flex"} items-center text-sm md:text-sm   gap-3  text-font-main-sub`}
          >
